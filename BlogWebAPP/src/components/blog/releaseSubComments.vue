<template>
    <div class="release-comment-container">
        <quill-editor
            v-focus="this.$store.state.ModulePopup.subCommentPopup"
            ref="commentsEditor"
            v-model="content"
            ...
            releaseEvent() {
                if(this.content !== '') {
                    let url = process.env.ROOT_API + 'comments/receiveSubComments.do',
                        commentsID = this.$store.state.ModuleOtherComment.fatherCommentId,
                        subCommentsID = this.$store.state.ModuleOtherComment.commentId,
                        param = {articleID: this.$route.query.id, content: this.comment, commentsID, subCommentsID},
                        success = resp => {
                            this.$toast("发表成功");
                            this.$store.commit("togglePopup", {subCommentPopup: false});
